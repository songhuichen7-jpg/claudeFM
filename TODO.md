...
- [x] E6. 🔴 **直接点歌 parse bug（全链路实测发现）**：`router.ts` 中文正则把「放 X 的 Y」解析成 title=X/artist=Y，但中文「X 的 Y」= Y by X（X 是艺人）——和英文「X by Y」操作数相反却共用赋值。导致**最自然的中文点歌「放周杰伦的晴天」艺人歌名反转、搜不到**。修：Intent 改带 title+artist 分开传 `resolvePlayableTrack`，中文「的」分支 swap（且 `的` 无需空格，覆盖「放周杰伦的晴天」）。临时 server 实测：放周杰伦的晴天→「晴天·周杰伦」✓、放林俊杰的可惜没如果→「可惜没如果·林俊杰」✓、英文 by 一致 ✓；不可播显诚实「找到了…但没播放链接」。属内部逻辑修复不改 server 契约（红线①不破）。
- [x] E7. ✨ **SettingsView 表单细节**：① `handleSave` 失败用原生 `alert()`（刺眼、不一致）→ 改 per-file 内联 `text-rose-400`（与 npErr/NCM error 一致）；② 新建 profile 的 id 输入按 placeholder 承诺过滤到 `a-z0-9_-`（即时反馈，不等服务端报错）。
- [x] E8. ✅ **发消息全链路实测**（Playwright）：用户消息立即上屏(乐观)→"CLAUDIO IS TUNING…"→/api/chat 200→DJ 逐字回复→ttsPending 异步→console 零错误，全链路通。借此挖出 E6。
- [x] E9. 🔴 **Library 播放保存的歌静默失效（交互审计发现）**：`/api/liked` 返回的收藏只有 `{id,title,artist}` **无 url**（NCM url 易变不存库），而 `selectTrack` 只在 `if (t.url)` 时播、`playTrack` 也对无 url 早返回——**点 Library 里往期收藏的歌什么都不放**（Library 核心动作坏掉）。修：`selectTrack` 无 url 时走 `resolveAndPlayTrack`（先 NCM 解析再播）。同时惠及聊天曲目卡。Playwright 实测：点歌→收藏→刷新→Library 点击→重新解析并播放(paused:false)✓。
- [ ] E10. 🟡 观察（待定）：Library 里 unlike 无撤销——Toast 是 `pointer-events-none` 胶囊做不了撤销，误点会丢收藏（已有「从品味里移除」toast 反馈）。ProfileCard 是静态人设卡、modal 礼仪全；浮层共性 a11y：无焦点陷阱（focus trap），Tab 会跑到背景。两者都需更大改动，列为 follow-up。
