...
/**
 * The user's saved taste — every ♥'d track. Reachable from the header heart.
 * Same austere mono/green language as the rest.
 */
export function LibraryView({ open, onClose }: Props) {
  const { liked, toggleLike, selectTrack, currentTrack } = usePrototype()
  ...
  const likedTracks = mockTracks.filter(t => liked[t.id])
