...
export function FocusView({ open, onClose }: Props) {
  const { currentTrack, currentTime, isPlaying, togglePlay, messages, activeDJId, djElapsedMs } = usePrototype()
...
/** Deep, calm cosmic backdrop — built from layered low-alpha radials + a
 * star-dot texture. Atmospheric, not a flat "AI gradient" blob. */
function Atmosphere() {
  return (
